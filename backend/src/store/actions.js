...
export function deleteCategory({commit}, category) {
  return axiosClient.delete(`/category/${category.id}`)
}

// Subcategories
export function getSubcategories({commit, state}, {url = null, search = '', per_page, sort_field, sort_direction} = {}) {
  commit('setSubcategories', [true])
  url = url || '/subcategory'
  const params = {
    per_page: state.subcategories.limit,
  }
  return axiosClient.get(url, {
    params: {
      ...params,
    ...
    .then((response) => {
      commit('setSubcategories', [false, response.data])
    })
    .catch(() => {
      commit('setSubcategories', [false])
    })
}

export function getSubcategory({commit}, id) {
  return axiosClient.get(`/subcategory/${id}`)
}

export function createSubcategory({commit}, subcategory) {
  return axiosClient.post('/subcategory', subcategory)
}

export function updateSubcategory({commit}, subcategory) {
  return axiosClient.put(`/subcategory/${subcategory.id}`, subcategory)
}

export function deleteSubcategory({commit}, subcategory) {
  return axiosClient.delete(`/subcategory/${subcategory.id}`)
}
