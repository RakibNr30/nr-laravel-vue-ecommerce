            ...
            <DialogPanel
              ...
              <header class="py-3 px-4 flex justify-between items-center">
                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                  {{ subcategory.id ? `Update subcategory: "${props.subcategory.name}"` : 'Create new Subcategory' }}
                </DialogTitle>
                <button
                  @click="closeModal()"
              ...
              <form @submit.prevent="onSubmit">
                <div class="bg-white px-4 pt-5 pb-4">
                  <CustomInput class="mb-2" v-model="subcategory.name" required="true" label="Subcategory Name"/>
                  <CustomInput type="textarea" class="mb-2" v-model="subcategory.description" label="Description"/>
                  <CustomInput type="select" class="mb-2" :select-options="categories" v-model="subcategory.category_id" required="true" label="Category"/>
                </div>
...
function onSubmit() {
  ...
  if (subcategory.value.id) {
    store.dispatch('updateSubcategory', subcategory.value)
      .then(response => {
        loading.value = false;
    ...
    store.dispatch('createSubcategory', subcategory.value)
      .then(response => {
        loading.value = false;
